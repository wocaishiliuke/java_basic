...
> - 因为有包的类的类名是全类名com.test.hello.Hello，此时需要手动创建包文件夹，并把.class文件放入到包下，然后在包文件夹外执行javac com.test.hello.Hello才可以
> - 在javac命令中使用-d参数，可将.class自动生成到包文件夹下,具体如下：
...
> - 同一包下，在一个类中访问另一个类，如果被需要的类没有编译成class文件，则会自动先编译被需要的类，再编译该类；但不同包下，不会自动去编译需要手动编译
> - 不同包下，类的使用需要全类名
